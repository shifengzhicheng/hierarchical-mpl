...
class LIBLEIDENALG_EXPORT GraphHelper
{
  public:
    GraphHelper(igraph_t* graph,
    ...
    GraphHelper(igraph_t* graph,
      vector<double> const& edge_weights,
      vector<double> const& node_sizes);
    GraphHelper(igraph_t* graph, int correct_self_loops);
    GraphHelper(igraph_t* graph);
    GraphHelper();
    ~GraphHelper();

    static GraphHelper* GraphFromEdgeWeights(igraph_t* graph, vector<double> const& edge_weights, int correct_self_loops);
    static GraphHelper* GraphFromEdgeWeights(igraph_t* graph, vector<double> const& edge_weights);
    static GraphHelper* GraphFromNodeSizes(igraph_t* graph, vector<double> const& node_sizes, int correct_self_loops);
    static GraphHelper* GraphFromNodeSizes(igraph_t* graph, vector<double> const& node_sizes);

    int has_self_loops();
    double possible_edges();
    double possible_edges(double n);

    GraphHelper* collapse_graph(MutableVertexPartition* partition);
