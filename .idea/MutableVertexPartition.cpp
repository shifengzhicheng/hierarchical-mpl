...
/****************************************************************************
  ...
  Parameters:
    graph            -- The igraph.GraphHelper on which this partition is defined.
...
MutableVertexPartition::MutableVertexPartition(GraphHelper* graph,
...
MutableVertexPartition* MutableVertexPartition::create(GraphHelper* graph)
{
  return new MutableVertexPartition(graph);
}

MutableVertexPartition* MutableVertexPartition::create(GraphHelper* graph, vector<size_t> const& membership)
{
...
void MutableVertexPartition::cache_neigh_communities(size_t v, igraph_neimode_t mode)
{
  ...
  #ifdef DEBUG
    cerr << "exit GraphHelper::cache_neigh_communities(" << v << ", " << mode << ")." << endl;
  #endif
}
