...
GraphHelper::GraphHelper(igraph_t* graph,
...
GraphHelper* GraphHelper::GraphFromEdgeWeights(igraph_t* graph, vector<double> const& edge_weights, int correct_self_loops)
{
  GraphHelper* g = new GraphHelper(graph, correct_self_loops);
...
GraphHelper* GraphHelper::GraphFromEdgeWeights(igraph_t* graph, vector<double> const& edge_weights)
{
  GraphHelper* g = new GraphHelper(graph);
...
GraphHelper* GraphHelper::GraphFromNodeSizes(igraph_t* graph, vector<double> const& node_sizes, int correct_self_loops)
{
  GraphHelper* g = new GraphHelper(graph, correct_self_loops);
...
GraphHelper* GraphHelper::GraphFromNodeSizes(igraph_t* graph, vector<double> const& node_sizes)
{
  GraphHelper* g = new GraphHelper(graph);
...
GraphHelper::GraphHelper(igraph_t* graph, int correct_self_loops)
{
  this->_graph = graph;
  this->_remove_graph = false;
  this->_correct_self_loops = correct_self_loops;
...
int GraphHelper::has_self_loops()
{
...
double GraphHelper::possible_edges()
{
  return this->possible_edges(this->vcount());
}

double GraphHelper::possible_edges(double n)
{
...
void GraphHelper::set_defaults()
{
  this->set_default_edge_weight();
  this->set_default_node_size();
}

void GraphHelper::set_default_edge_weight()
{
...
void GraphHelper::set_default_node_size()
{
...
void GraphHelper::set_self_weights()
{
  size_t n = this->vcount();
...
void GraphHelper::cache_neighbour_edges(size_t v, igraph_neimode_t mode)
{
  #ifdef DEBUG
    cerr << "void GraphHelper::cache_neighbour_edges(" << v << ", " << mode << ");" << endl;
  #endif
  ...
  #ifdef DEBUG
    cerr << "exit void GraphHelper::cache_neighbour_edges(" << v << ", " << mode << ");" << endl;
  #endif
}

vector<size_t> const& GraphHelper::get_neighbour_edges(size_t v, igraph_neimode_t mode)
{
  if (!this->is_directed())
...
void GraphHelper::cache_neighbours(size_t v, igraph_neimode_t mode)
{
  #ifdef DEBUG
    cerr << "void GraphHelper::cache_neighbours(" << v << ", " << mode << ");" << endl;
  #endif
  size_t degree = this->degree(v, mode);
  #ifdef DEBUG
    cerr << "Degree: " << degree << endl;
  ...
  #ifdef DEBUG
    cerr << "exit void GraphHelper::cache_neighbours(" << v << ", " << mode << ");" << endl;
  #endif
}

vector< size_t > const& GraphHelper::get_neighbours(size_t v, igraph_neimode_t mode)
{
  if (!this->is_directed())
    mode = IGRAPH_ALL; // igraph ignores mode for undirected graphs
 ...
 ********************************************************************************/
size_t GraphHelper::get_random_neighbour(size_t v, igraph_neimode_t mode, igraph_rng_t* rng)
{
  size_t node=v;
  size_t rand_neigh = -1;
...
/****************************************************************************
  ...
  graph.
*****************************************************************************/
GraphHelper* GraphHelper::collapse_graph(MutableVertexPartition* partition)
{
  #ifdef DEBUG
    cerr << "GraphHelper* GraphHelper::collapse_graph(vector<size_t> membership)" << endl;
  #endif
  ...
  GraphHelper* G = new GraphHelper(graph, collapsed_weights, csizes, this->_correct_self_loops);
  G->_remove_graph = true;
  #ifdef DEBUG
    cerr << "exit GraphHelper::collapse_graph(vector<size_t> membership)" << endl << endl;
  #endif
  return G;
}
}